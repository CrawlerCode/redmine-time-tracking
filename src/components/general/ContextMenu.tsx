            ...
            <>
              {(menu as MenuItem[][]).map((group, i) => (
                <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" key={i}>
                  {group.map((item) => (
                    <li className={clsx("flex px-2 py-1", item.disabled ? "text-text-disabled" : "hover:bg-background-hover")} onClick={!item.disabled ? item.onClick : undefined} key={item.name}>
