      ...
      <Modal title={formatMessage({ id: "issues.modal.add-issue.title" })} onClose={onClose}>
        <Formik<TCreateIssue>
          ...
          validationSchema={Yup.object({
            ...
            fixed_version_id: Yup.number(),
            assigned_to_id: Yup.number(),
            start_date: Yup.date(),
            due_date: Yup.date(),
            estimated_hours: Yup.number().min(0.01, formatMessage({ id: "issues.issue.field.estimated-hours.validation.greater-than-zero" })),
          })}
                  ...
                  <div className="grid grid-cols-2 gap-x-2">
                    <div className="flex flex-col gap-y-2">
                      ...
                      {(hasTrackerNoEnabledFields || selectedTracker?.enabled_standard_fields?.includes("category_id")) && (
                        <FastField type="select" name="category_id" error={touched.category_id && errors.category_id} as={CategoryField} size="sm" projectId={projectId} shouldUpdate={shouldUpdate} />
                      )}
