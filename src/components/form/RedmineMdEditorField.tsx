...
export const RedmineMdEditorField = ({ title, required, className, attachments, uploads, onUploadImage, ...props }: RedmineTextEditorFieldProps) => {
  ...
  const editorPortalNode = useMemo(() => createHtmlPortalNode(), []);
  ...
  return (
    <Field data-invalid={isInvalid} className={className}>
          ...
          extraCommands={[
                ...
                },
            {
              ...
              buttonProps: {
                ...
                title: formatMessage({ id: fullscreen ? "editor.command.fullscreen-exit" : "editor.command.fullscreen" }),
              },
              icon: fullscreen ? <Minimize2Icon className="size-4" /> : <Maximize2Icon className="size-4" />,
              execute: () => setFullscreen((prev) => !prev),
            },
          ]}
          height={150}
          {...props}
      ...
      {!fullscreen && <OutPortal node={editorPortalNode} />}
      <Dialog open={fullscreen} onOpenChange={setFullscreen}>
        <DialogContent className="size-[calc(100%-1rem)] max-w-none rounded-lg sm:max-w-none" showCloseButton={false}>
